...
function cosineDistance(a: number[], b: number[]): number {
  ...
  if (magnitudeA === 0 || magnitudeB === 0) {
    return 1; // Maximum cosine distance for zero vectors
  }

  const cosineSimilarity = dotProduct / (magnitudeA * magnitudeB);
  // Clamp to [-1, 1] to handle floating point errors
  const clampedSimilarity = Math.max(-1, Math.min(1, cosineSimilarity));
  return 1 - clampedSimilarity; // Convert similarity to distance
}
...
export class HDBSCAN {
  ...
  private metric: "euclidean" | "cosine";
  ...
  constructor({
    ...
    this.distanceFunction =
      metric === "cosine" ? cosineDistance : euclideanDistance;
  }
  ...
  private computeMutualReachabilityDistance(data: number[][]): number[][] {
    ...
    for (let i = 0; i < n; i++) {
      ...
      for (let j = 0; j < n; j++) {
        if (i !== j) {
          pointDistances.push({
            index: j,
            distance: this.distanceFunction(data[i], data[j]),
          });
        }
      }
    ...
    for (let i = 0; i < n; i++) {
      for (let j = 0; j < n; j++) {
        const directDistance = this.distanceFunction(data[i], data[j]);
    ...
    const processCluster = (cluster: Cluster) => {
      ...
      if (
        ...
        childClusters
          .filter(
            (c) =>
              c.size >= this.minClusterSize && !discardedClusters.has(c.id),
          )
